...
const DiaryModal = lazy(() => import("./DiaryModal"));
      ...
      {!hasEntries ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 gap-8">
          <button
            ...
            className="w-24 h-24 rounded-full flex items-center justify-center text-4xl shadow-2xl transition-transform duration-200 hover:scale-105 active:scale-95 animate-pulse-slow"
            style={{
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button
                onClick={openCurrentWeek}
                className="w-12 h-12 rounded-2xl flex items-center justify-center text-xl shadow-lg transition-transform duration-200 hover:scale-105 active:scale-95"
          ...
          {showNewEntry && (
              <div className={`${GLASS} p-4 flex items-center justify-center gap-3 animate-fade-in`}>
                <input type="date" value={newEntryDate} onChange={(e) => setNewEntryDate(e.target.value)}
                  max={new Date().toISOString().split("T")[0]} min={birthdate}
          ...
          {filtered.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {filtered.map((e) => (
                <div key={e.week_index} className={`${GLASS} p-5 group cursor-pointer animate-fade-in`}
            ...
            <div className="flex flex-col gap-2">
              {filtered.map((e) => (
                <div key={e.week_index} className={`${GLASS} !rounded-2xl px-4 py-3 flex items-center gap-4 group cursor-pointer animate-fade-in`}
                  onClick={() => openModalForWeek(e.week_index)}>
