...
const GridCell: React.FC<CellProps> = React.memo(
  ({ weekIndex, row, col, isCurrent, isPast, isDecadeMarker, hasDiary, dateLabel, weeksPassed, preview, onHover, onWeekClick }) => {

    const handleClick = () => { onWeekClick(weekIndex, row, col); };
    ...
    const handleMouseEnter = (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      const html = `
        <div class="text-left">
          ...
          <div class="text-text-muted mb-0.5 text-[0.8em]">${esc(dateLabel)}</div>
          <div class="text-[0.8em] font-medium ${statusColor} mb-1">${status}</div>
          ${preview ? `<div class="mt-1 pt-1 border-t border-[rgba(255,255,255,0.15)] italic text-text-main max-w-[200px] text-[0.8em] leading-snug">${esc(preview)}</div>` : ""}
    ...
    const statusText = isCurrent ? "Current week." : isPast ? "Past week." : "Future week.";
    const diaryText = hasDiary ? "Contains a diary entry." : "No diary entry yet.";
    const interactionText = weekIndex <= weeksPassed ? "Press Enter or Space to edit this week." : "Read only.";
    ...
    return (
      <div
        ...
        onClick={handleClick}
        onKeyDown={(e) => {
          if ((e.key === "Enter" || e.key === " ") && weekIndex <= weeksPassed) {
            e.preventDefault();
            handleClick();
          }
        }}
        role="button"
        tabIndex={0}
        aria-disabled={weekIndex > weeksPassed}
        aria-label={`Year ${row}, Week ${col + 1}, starting ${dateLabel}. ${statusText} ${diaryText} ${interactionText}`}
      />
    );
  },
);
...
const WeeksGrid: React.FC<WeeksGridProps> = ({ weeksPassed, totalYears, birthdate, onHover, onWeekClick, diaryEntries, scale }) => {
  ...
  // Pre-compute all date labels once instead of per-cell per-render
  const dateLabels = React.useMemo(() => {
    const birth = new Date(birthdate);
    if (isNaN(birth.getTime())) return null;
    const total = totalYears * VISIBLE_WEEKS;
    const labels = new Array<string>(total);
    for (let i = 0; i < total; i++) {
      labels[i] = format(addWeeks(birth, i), "MMM d, yyyy");
    }
    return labels;
  ...
  return (
    <div className="relative w-full overflow-x-auto mb-4 sm:mb-6">
      ...
      >
        <div
          className="inline-flex flex-col animate-fade-in min-w-min mx-auto border border-box-border p-1 sm:p-2 bg-[rgba(0,0,0,0.05)] rounded-sm"
          role="grid"
          aria-label="Life in weeks grid. Each cell represents one week of life."
        >
          {/* Header row */}
          <div className="flex items-center gap-px" role="row">
            <div className="min-w-[22px] sm:min-w-[25px] lg:min-w-[28px] h-[18px] sm:h-[20px] lg:h-[22px]" aria-hidden="true" />
          ...
          {Array.from({ length: totalYears }, (_, row) => (
            <div key={row} className="flex items-center gap-px" role="row">
              <div
                className="text-primary font-medium min-w-[22px] sm:min-w-[25px] lg:min-w-[28px] h-[18px] sm:h-[20px] lg:h-[22px] text-[9px] sm:text-[10px] lg:text-[11px] flex items-center justify-center pr-1"
                aria-hidden="true"
              >
                {row}
              </div>
              {Array.from({ length: VISIBLE_WEEKS }, (_, col) => {
                ...
                const entry = diaryEntries[idx.toString()];
                ...
                return (
                  <GridCell
                    ...
                    hasDiary={!!entry}
                    dateLabel={dateLabels?.[idx] ?? "Unknown date"}
